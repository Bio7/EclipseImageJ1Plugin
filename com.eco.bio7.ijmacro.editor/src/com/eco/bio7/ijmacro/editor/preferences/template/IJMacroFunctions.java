...
public class IJMacroFunctions {
	...
	public static String functions=""+
			...
			"getPixel(x,y)####Returns the value of the pixel at (x,y). Note that pixels in RGB images contain red,green and blue components that need to be extracted using shifting and masking. See the Color Picker Tool macro for an example that shows how to do this."+linSeparator + 
			...
			"setOption(option,boolean)####Enables or disables an ImageJ option,where option is one of the following options and boolean is either true or false. \"AutoContrast\" enables/disables the Edit>Options>Appearance \"Auto contrast stacks\" option. You can also have newly displayed stack slices contrast enhanced by holding the shift key down when navigating stacks. \"Bicubic\" provides a way to force commands like Edit>Selection>Straighten,that normally use bilinear interpolation,to use bicubic interpolation. \"BlackBackground\" enables/disables the Process>Binary>Options \"Black background\" option. \"Changes\" sets/resets the 'changes' flag of the current image. Set this option false to avoid \"Save Changes?\" dialog boxes when closing images. \"DebugMode\" enables/disables the ImageJ debug mode. ImageJ displays information,such as TIFF tag values,when it is in debug mode. \"DisablePopupMenu\" enables/disables the the menu displayed when you right click on an image. \"DisableUndo\" enables/disables the Edit>Undo command. Note that a setOption(\"DisableUndo\",true) call without a corresponding setOption(\"DisableUndo\",false) will cause Edit>Undo to not work as expected until ImageJ is restarted. \"Display label\",\"Limit to threshold\",\"Area\",\"Mean\" and \"Std\",\"Perimeter\",\"Stack position\" and \"Add to overlay\" enable/disable the corresponding Analyze>Set Measurements options. \"ExpandableArrays\" enables/disables support for auto-expanding arrays. Note that macros that use auto-expanding arrays will not be compatible with Image 2.0. \"JFileChooser\" enables/disables use of the Java JFileChooser to open and save files instead of the native OS file chooser. \"Loop\" enables/disables the Image>Stacks>Tools>Animation Options \"Loop back and forth\" option. \"OpenUsingPlugins\" controls whether standard file types (TIFF,JPEG,GIF,etc.) are opened by external plugins or by ImageJ. \"QueueMacros\" controls whether macros invoked using keyboard shortcuts run sequentially on the event dispatch thread (EDT) or in separate,possibly concurrent,threads. In \"QueueMacros\" mode,screen updates,which also run on the EDT,are delayed until the macro finishes. \"Show All\" enables/disables the the \"Show All\" mode in the ROI Manager. \"ShowAngle\" determines whether or not the \"Angle\" value is displayed in the Results window when measuring straight line lengths. Requires 1.49c. \"ShowMin\" determines whether or not the \"Min\" value is displayed in the Results window when \"Min & Max Gray Value\" is enabled in the Analyze>Set Measurements dialog box. \"ShowRowNumbers\" enables/disables display of Results table row numbers. \"AntialiasedText\" controls the \"Antialiased text\" option in the Edit>Options>Fonts dialog. Requires v1.51h. \"WandAllPoints\" Controls whether Wand selections with straight lines longer than one pixel should have intermediate points with single-pixel spacing. Requires v1.51q. \"ScaleConversions\" Enables/disables the \"Scale when converting\" option in the Edit>Options>Conversons dialog. Requires v1.52k. \"SupportMacroUndo\" undo a macro operation."+linSeparator+
			"setOption(\"CopyHeadings\",boolean)####Enables the copy of the table header."+linSeparator+	
			"setOption(\"InterpolateLines\",boolean)####Enables the interpolation of lines."+linSeparator+
			"setOption(\"FlipFitsImages\",true)####Sets the option to flip images of type 'Fits'."+linSeparator+
			"setOption(\"CalibrateConversions\",boolean)####Sets the option to calibrate the conversions."+linSeparator+
