...
public class Menus {
	...
	private static CheckboxMenuItem gray8Item, gray16Item, gray32Item, color256Item, colorRGBItem, RGBStackItem,
			HSBStackItem, LabStackItem, HSB32Item;
	...
	/* Changed for Bio7! */
	public static Vector<String> bio7JarAllCommand;
	...
	String addMenuBar() {
		...
		/* Changed for Bio7! */
		bio7JarAllCommand = new Vector<String>();
		...
		addPlugInItem(help, "Mailing List...",
				"ij.plugin.BrowserLauncher(\"https://list.nih.gov/archives/imagej.html\")", 0, false);
		help.addSeparator();
		...
		// addPlugInItem(help, "Update ImageJ...", "ij.plugin.ImageJ_Updater",
		// 0, false);
	...
	void setupPluginsAndMacrosPaths() {
		 ...
		 * pluginsDir + File.separator + "ImageJ"; // needed to run plugins when ImageJ
		 * launched using Java WebStart if (applet==null)
	...
	public static void updateBio7ImageJMenu() {
		Display dis = Util.getDisplay();
		dis.asyncExec(new Runnable() {
			public void run() {
				CanvasView canvasView = CanvasView.getCanvas_view();
				IViewSite viewSite = canvasView.getViewSite();
				IActionBars actionBars = viewSite.getActionBars();
				IContributionItem[] it = actionBars.getToolBarManager().getItems();
				for (int j = 0; j < it.length; j++) {
					//String name = Menus.getMenuBar().getMenu(j).getName();
					String name = it[j].getId();
					if (name.equals("File") == false && name.equals("Edit") == false && name.equals("Image") == false
							&& name.equals("Process") == false && name.equals("Analyze") == false
							&& name.equals("Plugins") == false && name.equals("Window") == false
							&& name.equals("Help") == false) {

						actionBars.getToolBarManager().remove(name);
						actionBars.getToolBarManager().update(true);
					}
				}
				int count = Menus.getMenuBar().getMenuCount();
				for (int i = 0; i < count; i++) {
					String name = Menus.getMenuBar().getMenu(i).getLabel();
					if (name.equals("File") == false && name.equals("Edit") == false && name.equals("Image") == false
							&& name.equals("Process") == false && name.equals("Analyze") == false
							&& name.equals("Plugins") == false && name.equals("Window") == false
							&& name.equals("Help") == false) {
						if (name.isEmpty() == false) {
							actionBars.getToolBarManager().add(new ImageJExtensionAction(name));
							actionBars.getToolBarManager().update(true);
						}
					}
				}
