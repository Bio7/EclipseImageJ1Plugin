 ...
 */
public class PlotWindow extends ImageWindow
	...
	/**
	 * Have axes with no grid lines. If both noGridLines and noTicks are true, only
	...
	/**
	 * Creates a PlotWindow from a given ImagePlus with a Plot object. (called when
	 * reading an ImagePlus with an associated plot from a file)
	 */
	 ...
	 * the plot is displayed. Note that there are more options available by using
	 * the methods of the Plot class instead.
	 * 
	 * @deprecated use the corresponding method of the Plot class
	 */
	...
	/**
	 * Adds vertical error bars to the plot. Must be called before the plot is
	 * displayed. Note that there are more options available by using the methods of
	 * the Plot class instead.
	 * 
	 * @deprecated use the corresponding method of the Plot class
	 */
	...
	/**
	 * Draws a label. Note that there are more options available by using the
	 * methods of the Plot class instead.
	 * 
	 * @deprecated use the corresponding method of the Plot class
	 */
	 ...
	 * Must be called before the plot is displayed. Note that there are more options
	 * available by using the methods of the Plot class instead.
	 * 
	 * @deprecated use the corresponding method of the Plot class
	 */
	...
	/**
	 * Changes the line width. Must be called before the plot is displayed. Note
	 * that there are more options available by using the methods of the Plot class
	 * instead.
	 * 
	 * @deprecated use the corresponding method of the Plot class
	 */
	...
	/**
	 * Changes the font. Must be called before the plot is displayed. Note that
	 * there are more options available by using the methods of the Plot class
	 * instead.
	 * 
	 * @deprecated use the corresponding method of the Plot class
	 */
	...
	public void draw() {
		...
		bottomPanel.setBackground(swtBackgroundToAWT);
		bottomPanel.setForeground(swtForegroundToAWT);
		//}
		...
		data.setBackground(swtBackgroundToAWT);
		data.setForeground(swtForegroundToAWT);
		data.setBorderPainted(false);
		data.addActionListener(this);
		bottomPanel.add(data);
		more = new JButton(moreButtonLabel);
		more.setBackground(swtBackgroundToAWT);
		more.setForeground(swtForegroundToAWT);
		...
		// Changed for Bio7!
		//statusLabel.setBackground(primarySWTToAwtColor);
		/*
		 * Changed for Bio7! AWT must be colored per component. All JPanels will get the
		 * SWT background from the definition of the CanvasView (UIManager)!
		 */
		//if (Util.getOS().equals("Linux")) {
		statusLabel.setBackground(swtBackgroundToAWT);
		statusLabel.setForeground(swtForegroundToAWT);
		//}
	...
	/**
	 * Called when the window is activated (WindowListener) Window layout is
	 * finished at latest a few millisec after windowActivated, then the
	 * 'wasActivated' boolean is set to tell the ImageCanvas that resize events
	 * should lead to resizing the canvas (before, creating the layout can lead to
	 * resize events)
	 */
	...
	/**
	 * Shows the text in the coordinate&status readout field at the bottom. This
	 * text may get temporarily replaced for 'tooltips' (mouse over range arrows
	 * etc.). Call with a null argument to enable coordinate readout again.
	 */
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (b == live) {
			...
			ImageJContextMenuAction icma = instance.getImageContextMenuAction();
			icma.swtPopupMenu(e, dataPopupMenu, new Point(loc.x, loc.y), this);
		} else if (b == more) {
			enableDisableMenuItems();
			Component c = (Component) b;
			Point loc = c.getLocationOnScreen();
			...
			/*Changed for Bio7*/
			ImageJ instance = IJ.getInstance();
			ImageJContextMenuAction imageContextMenuAction = instance.getImageContextMenuAction();
			imageContextMenuAction.swtPopupMenu(e, morePopupMenu, new Point(loc.x, loc.y), this);
		}

		actionPerformed(b);
	}

	/*Changed for Bio7 - see above!*/
	/** Called if user has activated a button or popup menu item */
	public void actionPerformed(Object b) {
		try {
	...
	/**
	 * Creates an overlay with triangular buttons and other symbols for changing the
	 * axis range limits and shows it
	 */
	...
	/**
	 * Returns the index of the range-modifying symbol or axis at the cursor
	 * position x,y, or -1 of none. Index numbers for arrows start with 0 at the
	 * 'down' arrow of the lower side of the x axis and end with 7 the up arrow at
	 * the upper side of the y axis. Numbers 8 & 9 are for "Reset Range" and "Fit
	 * All"; numbers 10-13 for a dialog to set a single limit, and 14-15 for the
	 * axis options.
	 */
	 ...
	 * as a ResultsTable. Use plot.getResultsTableWithLabels for a table with data
	 * set labels as column headings
	 */
