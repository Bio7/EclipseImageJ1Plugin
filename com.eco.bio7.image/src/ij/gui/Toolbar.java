...
public class Toolbar extends JPanel implements MouseListener, MouseMotionListener, ItemListener, ActionListener {
	...
	private static final int SIZE = 34; // no longer used
	...
	private void fill3DRect(Graphics g, int x, int y, int width, int height, boolean raised) {
		...
		if (raised)
			...
			/* Changed for Bio7! Instead of: darker! */
			if (Util.isThemeBlack()) {
				g.setColor(gray);
			} else {
				g.setColor(darker);
	...
	public void triggerPopupMenu(int newTool, MouseEvent e, boolean isRightClick, boolean isLongPress) {
		...
		if (current == OVAL && isRightClick) {
			...
			//swtPopupMenu(e, ovalPopup);
			ImageJ instance = IJ.getInstance();
			ImageJContextMenuAction icma = instance.getImageContextMenuAction();
			icma.swtPopupMenu(e, ovalPopup, this);
			//ovalPopup.show(e.getComponent(), x, y);
			mouseDownTime = 0L;
		}
		if (current == POINT && isRightClick) {
			...
			//swtPopupMenu(e, pointPopup);
			ImageJ instance = IJ.getInstance();
			ImageJContextMenuAction icma = instance.getImageContextMenuAction();
			icma.swtPopupMenu(e, pointPopup, this);
			//pointPopup.show(e.getComponent(), x, y);
			mouseDownTime = 0L;
		}
		if (isLine(current) && isRightClick) {
			...
			/*Changed for Bio7!*/
			// ovalPopup.show(e.getComponent(), x, y);
			//swtPopupMenu(e, linePopup);
			ImageJ instance = IJ.getInstance();
			ImageJContextMenuAction icma = instance.getImageContextMenuAction();
			icma.swtPopupMenu(e, linePopup, this);
			//linePopup.show(e.getComponent(), x, y);
			mouseDownTime = 0L;
		}
		if (current == MAGNIFIER && isRightClick) {
			/*Changed for Bio7!*/
			//swtPopupMenu(e, pickerPopup);
			ImageJ instance = IJ.getInstance();
			ImageJContextMenuAction icma = instance.getImageContextMenuAction();
			icma.swtPopupMenu(e, zoomPopup, this);
		...
		if (current == DROPPER && isRightClick) {
			...
			ImageJ instance = IJ.getInstance();
			ImageJContextMenuAction icma = instance.getImageContextMenuAction();
			icma.swtPopupMenu(e, pickerPopup, this);
			mouseDownTime = 0L;
		}
	...
	public void mousePressed(final MouseEvent e) {
		...
		if (menus[newTool] != null&& menus[newTool].getItemCount()>0) {
			...
			ImageJ instance = IJ.getInstance();
			ImageJContextMenuAction icma = instance.getImageContextMenuAction();
			icma.swtPopupMenu(e, menus[newTool], this);
			return;
		}
	...
	void showSwitchPopupMenu(MouseEvent e) {
		...
		// swtPopupMenu(e, switchPopup);

		ImageJ instance = IJ.getInstance();
		ImageJContextMenuAction icma = instance.getImageContextMenuAction();
		icma.swtPopupMenu(e, switchPopup, this);

	}

	/*Changed for Bio7! SWT Popup Menu!*/
	/*private void swtPopupMenu(MouseEvent e, PopupMenu popUp) {
		Point p = e.getLocationOnScreen();
	
		int count = popUp.getItemCount();
		Util.getDisplay().asyncExec(new Runnable() {
			public void run() {
				final org.eclipse.swt.widgets.Menu menu = new org.eclipse.swt.widgets.Menu(Util.getShell(), SWT.POP_UP);
						...
						for (int i = 0; i < count; i++) {
							...
							if (label.equals("-")) {
								...
								if (switchPopupItem instanceof CheckboxMenuItem) {
									...
									final org.eclipse.swt.widgets.MenuItem item = new org.eclipse.swt.widgets.MenuItem(menu, SWT.NONE);
									item.setText(label);
									item.addSelectionListener(new SelectionListener() {
	
										public void widgetSelected(SelectionEvent e) {
											actionPerformed(switchPopupItem);
										}
	...
	/*Changed for Bio7! to call menu!*/
	public void itemEvent(MenuItem item) {
		String previousName = getToolName();
	...
	/*Changed for Bio7!*/
	public void actionPerformed(MenuItem item) {
