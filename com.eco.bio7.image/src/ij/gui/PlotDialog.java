...
public class PlotDialog implements DialogListener {
	...
	/**
	 * Types of dialog. Note that 10-14 must be the same as the corresponding
	 * PlotWindow.rangeArrow numbers
	 */
	...
	private static final String[] HEADINGS = new String[] { "Plot Range", "Axis Options", "Add Legend", "High-Resolution Plot", "Use Template", null, null, null, null, null, // 5-9 spare
	...
	/**
	 * Template "copy what" flag: dialog texts and corresponding bit masks, in the
	 * sequence as they appear in the dialog
	 */
	...
	private static final int[] TEMPLATE_FLAGS = new int[] { Plot.X_RANGE, Plot.Y_RANGE, Plot.COPY_AXIS_STYLE, Plot.COPY_LABELS, Plot.COPY_LEGEND, Plot.COPY_CONTENTS_STYLE, Plot.COPY_EXTRA_OBJECTS,
			Plot.COPY_SIZE };
	...
	/**
	 * Asks the user for axis scaling; then replot with new scale on the same
	 * ImageProcessor. The 'parent' frame may be null
	 */
	public void showDialog(Frame parent) {
		...
		String dialogTitle = dialogType >= X_LEFT && dialogType <= Y_TOP ? "Set Axis Limit..." : (HEADINGS[dialogType] + "...");
		GenericDialog gd = parent == null ? new GenericDialog(dialogTitle) : new GenericDialog(dialogTitle, parent);
		if (!setupDialog(gd))
			return;
	...
	/**
	 * Setting up the dialog fields and initial parameters. The input is read in the
	 * dialogItemChanged method, which must have exactly the same structure of 'if'
	 * blocks and matching 'get' methods for each input field.
	 * 
	...
	private boolean setupDialog(GenericDialog gd) {
		...
		if (dialogType == LEGEND) {
			...
			if (legendFont == null)
				legendFont = (plot.currentFont != null) ? plot.currentFont : plot.defaultFont;
		...
		if (dialogType == TEMPLATE) {
			...
			if (lastTemplateFlags == 0)
				lastTemplateFlags = Plot.COPY_AXIS_STYLE | Plot.COPY_CONTENTS_STYLE;
	...
	/**
	 * This method is called when the user changes something in the dialog. Note
	 * that the 'if's for reading the fields must be exactly the same as those for
	 * setting up the fields in 'setupDialog' (fields must be also read in the same
	 * sequence).
	 */
	public boolean dialogItemChanged(GenericDialog gd, AWTEvent e) {
		...
		if (dialogType == AXIS_OPTIONS || dialogType == X_AXIS || dialogType == Y_AXIS) {
			...
			for (int l = 0; l < xFlags.length; l++) {
				...
				if (dialogType == AXIS_OPTIONS || dialogType == Y_AXIS)
					if (gd.getNextBoolean())
						flags |= xFlags[l] << 1; //y flags are shifted up one bit;
			}
		...
		if (dialogType == TEMPLATE) {
			...
			if (imp != null)
				templateID = imp.getID(); //remember for next time
			int templateFlags = 0;
			for (int i = 0; i < TEMPLATE_FLAGS.length; i++)
				if (gd.getNextBoolean())
	...
	private void record() {
		...
		if (dialogType == SET_RANGE || dialogType == X_AXIS || dialogType == Y_AXIS) {
			...
			int xDigits = plot.logXAxis ? -2 : Plot.getDigits(currentMinMax[0], currentMinMax[1], 0.005 * Math.abs(currentMinMax[1] - currentMinMax[0]), 6, 0);
			int yDigits = plot.logYAxis ? -2 : Plot.getDigits(currentMinMax[2], currentMinMax[3], 0.005 * Math.abs(currentMinMax[3] - currentMinMax[2]), 6, 0);
			Recorder.recordString(plotDot + "setLimits(" + IJ.d2s(currentMinMax[0], xDigits) + "," + IJ.d2s(currentMinMax[1], xDigits) + "," + IJ.d2s(currentMinMax[2], yDigits) + ","
					+ IJ.d2s(currentMinMax[3], yDigits) + ");\n");
		}
		if (dialogType == AXIS_OPTIONS || dialogType == X_AXIS || dialogType == Y_AXIS) {
			int flags = plot.getFlags();
		...
		if (dialogType == LEGEND) {
			...
			if (Recorder.scriptMode()) {
				...
				if (getFlag(lFlags, Plot.LEGEND_BOTTOM_UP))
					options += " Bottom-To-Top";
				if (getFlag(lFlags, Plot.LEGEND_TRANSPARENT))
					options += " Transparent";
	...
	private void doHighResolutionDialog(Frame parent) {
		GenericDialog gd = parent == null ? new GenericDialog(HEADINGS[dialogType]) : new GenericDialog(HEADINGS[dialogType], parent);
		...
		*//**
			 * The following command is needed to have the high-resolution plot as front
			 * window. Otherwise, as the dialog is owned by the original PlotWindow, the
	...
	/**
	 * Disables switching on a checkbox for log range if the axis limits do not
	 * allow it. The checkbox can be always switched off.
	 */
