 ...
 * </pre>
 * 
 * <b> Convention for subpixel resolution and zooming in: </b>
 * <ul>
 * <li>Area ROIs: Integer coordinates refer to the top-left corner of the pixel
 * with these coordinates. Thus, pixel (0,0) is enclosed by the rectangle
 * spanned between points (0,0) and (1,1), i.e., a rectangle at (0,0) with width
 * = height = 1 pixel.
 ...
 * of pixels (0,0) and (1,0), respectively, and drawing the line should affect
 * both pixels. For images dispplayed at high zoom levels, this means that
 * (open) lines and single points are displayed 0.5 pixels further to the right
 * and bottom than the outlines of area ROIs (closed lines) with the same
 * coordinates.
 * </ul>
 * Note that rectangular and (nonrotated) oval ROIs do not support subpixel
 * resolution. Since ImageJ 1.52t, this convention does not depend on the
 * Prefs.subpixelResolution (previously accessible via Edit>Options>Plot) and
 * this flag has no effect any more.
 *
 */
public class Roi extends Object implements Cloneable, java.io.Serializable, Iterable<Point> {
	...
	public static final int RECTANGLE = 0, OVAL = 1, POLYGON = 2, FREEROI = 3, TRACED_ROI = 4, LINE = 5, POLYLINE = 6,
			FREELINE = 7, ANGLE = 8, COMPOSITE = 9, POINT = 10; // Types
	...
	/**
	 * Starts the process of creating a user-defined rectangular Roi, where sx and
	 * sy are the starting screen coordinates.
	 */
	...
	/**
	 * Starts the process of creating a user-defined rectangular Roi, where sx and
	 * sy are the starting screen coordinates. For rectangular rois, also a corner
	 * diameter may be specified to make it a rounded rectangle
	 */
	...
	/**
	 * Sets the ImagePlus associated with this ROI. <code>imp</code> may be null to
	 * remove the association to an image.
	 */
	...
	/**
	 * Returns an array with the following values: <br>
	 * [0] "Feret" (maximum caliper width) <br>
	 * [1] "FeretAngle" (angle of diameter with maximum caliper width, between 0 and
	 * 180 deg) <br>
	 * [2] "MinFeret" (minimum caliper width) <br>
	 * [3][4] , "FeretX" and "FeretY", the X and Y coordinates of the starting point
	 * (leftmost point) of the maximum-caliper-width diameter. <br>
	 * [5-7] reserved <br>
	 ...
	 * caliper diameter, in unscaled image pixel coordinates: <br>
	 ...
	 * diameter. Both of these points are vertices of the convex hull. <br>
	 * The final four array elements are the starting and end points of the minimum
	 * caliper width, <br>
	 * [12],[13] "MinFeretX", "MinFeretY", and <br>
	 ...
	 * of the convex hull, while second point (MinFeretX2, MinFeretY2) usually is
	 * not a vertex point but at a boundary line of the convex hull.
	 */
	...
	/**
	 * Returns the convex hull of this Roi as a Polygon with integer coordinates by
	 * rounding the floating-point values. Coordinates of the convex hull are image
	 * pixel coordinates.
	 */
	...
	/**
	 * Returns the convex hull of this Roi as a FloatPolygon. Coordinates of the
	 * convex hull are image pixel coordinates.
	 */
	...
	/**
	 * Sets the bounds of rectangular, oval or text selections. Note that for these
	 * types, subpixel resolution is ignored, and the x,y values are rounded down,
	 * the width and height values rounded up. Do not use for other ROI types since
	 * their width and height are results of a calculation. For translating ROIs,
	 * use setLocation.
	 */
	...
	/**
	 * Returns the outline in image pixel coordinates, where options may include
	 * "close" to add a point to close the outline if this is an area roi and the
	 * outline is not closed yet. (For ShapeRois, "separate" inserts NaN values
	 * between subpaths).
	 */
	 ...
	 * position of a pixel center determines whether a pixel is selected. Points
	 * exactly at the left (right) border are considered outside (inside); points
	 * exactly on horizontal borders are considered outside (inside) at the border
	 ...
	 * converted to a ShapeRoi. If imp is not given, assumes a rectangle of size
	 * 2e9*2e9 for the boundary.
	 */
	...
	/**
	 * Sets the group value assigned to newly created ROIs, and also sets the
	 * default ROI color to the group color. Set to zero to not have a default group
	 * and to use the default ROI color.
	 * 
	 * @see #setGroup
	 * @see #getGroup
	 * @see #getGroupColor
	 ...
	 *             shifted by 0.5 pixels top the bottom and right, for better
	 *             agreement with the position used by ProfilePlot, with the default
	 *             taken from Prefs.subPixelResolution. Now the shift is independent
	 *             of this setting and only depends on the ROI type (area or
	 *             line/point ROI).
	 */
	@Deprecated
	 ...
	 * pixel coordinates, depending on whether this roi uses the line or area
