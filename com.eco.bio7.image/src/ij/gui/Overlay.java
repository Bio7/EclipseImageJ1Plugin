...
public class Overlay implements Iterable<Roi> {
	...
	/**
	 * Returns the index of the last ROI that contains the point (x,y) or null if no
	 * ROI contains the point.
	 */
	...
	/**
	 * Measures the ROIs in this overlay on the specified image and returns the
	 * results as a ResultsTable.
	 */
	...
	/*
	 * Duplicate the elements of this overlay which intersect with the rectangle
	 * 'bounds'. Author: Wilhelm Burger Author: Marcel Boeglin
	 */
	...
	/**
	 * Removes ROIs having positions outside of the interval defined by firstSlice
	 * and lastSlice. Marcel Boeglin, September 2013
	 */
	...
	/**
	 * Removes ROIs having a C, Z or T coordinate outside the volume defined by
	 * firstC, lastC, firstZ, lastZ, firstT and lastT. Marcel Boeglin, September
	 * 2013
	 */
	...
	/*
	 * public Rectangle getBounds() { if (size()==0) return new Rectangle(0,0,0,0);
	 * int xmin = Integer.MAX_VALUE; int xmax = -Integer.MAX_VALUE; int ymin =
	 * Integer.MAX_VALUE; int ymax = -Integer.MAX_VALUE; Roi[] rois = toArray(); for
	 * (int i=0; i<rois.length; i++) { Rectangle r = rois[i].getBounds(); if
	 * (r.x<xmin) xmin = r.x; if (r.y<ymin) ymin = r.y; if (r.x+r.width>xmax) xmax =
	 * r.x+r.width; if (r.y+r.height>ymax) ymax = r.y+r.height; } return new
	 * Rectangle(xmin, ymin, xmax-xmin, ymax-ymin); }
	 */

	/*
	 * Returns the Roi that results from XORing all the ROIs in this overlay that
	 * have an index in the array ‘indexes’.
	 */
	...
	/**
	 * Set the label font size with options. The options string can contain 'scale'
	 * (enlarge labels when image zoomed), 'bold' (display bold labels) or
	 * 'background' (display labels with contrasting background.
	 */
	...
	/**
	 * Fills all the ROIs in this overlay with 'foreground' after clearing the the
	 * image to 'background' if it is not null.
	 */
	...
	/**
	 * Set 'false' to prevent ROIs in this overlay from being activated by clicking
	 * on their labels or by a long clicking.
	 */
	...
	/**
	 * Returns 'true' if ROIs in this overlay can be activated by clicking on their
	 * labels or by a long press.
	 */
	...
	/**
	 * Updates overlays created by the particle analyzer after rows are deleted from
	 * the Results table.
	 */
