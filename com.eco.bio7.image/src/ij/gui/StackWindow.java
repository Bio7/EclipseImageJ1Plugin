...
public class StackWindow extends ImageWindow
	...
	public StackWindow(ImagePlus imp, ImageCanvas ic) {
		...
		/*
		 * pack(); ic = imp.getCanvas(); if (ic!=null) ic.setMaxBounds();
		 * show(); int previousSlice = imp.getCurrentSlice(); if
		 * (previousSlice>1 && previousSlice<=imp.getStackSize())
		 * imp.setSlice(previousSlice); else imp.setSlice(1); thread = new
		 * Thread(this, "zSelector"); thread.start();
		 */
