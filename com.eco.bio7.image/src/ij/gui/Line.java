...
public class Line extends Roi {
	...
	/**
	 * Creates a new straight line selection using the specified starting and ending
	 * offscreen integer coordinates.
	 */
	 ...
	 * 'sx' and 'sy' are screen coordinates that specify the start of the line. The
	 * user will determine the end of the line interactively using rubber banding.
	 */
	...
	/**
	 * Used for angle searches in line ROI creation: tan = y/x for angle limits
	 * 1/2*45 degrees, and 3/2*45 deg
	 */
	...
	/**
	 * Sets the x1d, y1d, x2d, y2d line end points, the (legacy) integer coordinates
	 * of the end points x1, y1, x2, y2 the 'bounds' subpixel rectangle of the Roi
	 * superclass (spanned by the end points), the int x, y, width, height integer
	 * bounds of the superclass (these enclose the 'draw' area for 1 pixel width),
	 * and the coordinates x1R, y1R, x2R, y2R relative to the base x, y of the
	 * 'bounds'
	 */
	...
	public void showStatus() {
		IJ.showStatus(imp.getLocationAsString((int) Math.round(x2d), (int) Math.round(y2d)) + ", angle=" + IJ.d2s(getAngle()) + ", length=" + IJ.d2s(getLength()));
	}
	...
	/**
	 * Returns the default line width. Use getStrokeWidth()to get the width of a
	 * Line instance.
	 * 
	 * @see #getStrokeWidth
	 */
	...
	/**
	 * Sets the default line width. Use setStrokeWidth() to set the width of a Line
	 * instance.
	 * 
	 * @see #setStrokeWidth
	 */
	...
	/**
	 * Sets the x coordinate of the leftmost and y coordinate of the topmost end
	 * point
	 */
