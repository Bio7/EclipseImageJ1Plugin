...
public class Calibration implements Cloneable {
	...
	/**
	 * Plugin writers can use this string to store information about the image. This
	 * string is saved in the TIFF header if it is not longer than 64 characters and
	 * it contains no '=' or '\n' characters.
	 */
	public String info;
	...
	/**
	 * Constructs a new Calibration object using the default values. For density
	 * calibration, the image is assumed to be 8-bits.
	 */
	public Calibration() {
	}
	...
	/**
	 * Converts a y-coordinate in pixels to physical units (e.g. mm), taking into
	 * account the invertY and global "Invert Y Coordinates" flags.
	 */
	...
	/**
	 * Converts a y-coodinate in physical units to pixels, taking into account the
	 * 'invertY' flag.
	 */
	...
	/**
	 * Disables the density calibation if the specified image has a differenent bit
	 * depth.
	 */
	...
	/**
	 * Returns the calibration table. With 8-bit images, the table has a length of
	 * 256. With 16-bit images, the length is 65536.
	 */
	...
	/**
	 * Sets the calibration table. With 8-bit images, the table must have a length
	 * of 256. With 16-bit images, it must be 65536.
	 */
	...
	public boolean isSigned16Bit() {
		return (bitDepth == 16 && function >= STRAIGHT_LINE && function <= EXP_RECOVERY && coefficients != null && coefficients[0] == -32768.0 && coefficients[1] == 1.0);
	}
	...
	public String toString() {
		return "w=" + pixelWidth + ", h=" + pixelHeight + ", d=" + pixelDepth + ", unit=" + unit + ", f=" + function + ", nc=" + (coefficients != null ? "" + coefficients.length : "null") + ", table="
				+ (cTable != null ? "" + cTable.length : "null") + ", vunit=" + valueUnit + ", bd=" + bitDepth;
	}
}
