 ...
 * values. Call the static ResultsTable.getResultsTable() method to get a
 * reference to the ResultsTable used by the <i>Analyze/Measure</i> command.
 * 
...
public class ResultsTable implements Cloneable {
	...
	public static final int AREA = 0, MEAN = 1, STD_DEV = 2, MODE = 3, MIN = 4, MAX = 5, X_CENTROID = 6, Y_CENTROID = 7,
			X_CENTER_OF_MASS = 8, Y_CENTER_OF_MASS = 9, PERIMETER = 10, ROI_X = 11, ROI_Y = 12, ROI_WIDTH = 13,
			ROI_HEIGHT = 14, MAJOR = 15, MINOR = 16, ANGLE = 17, CIRCULARITY = 18, FERET = 19, INTEGRATED_DENSITY = 20,
			MEDIAN = 21, SKEWNESS = 22, KURTOSIS = 23, AREA_FRACTION = 24, RAW_INTEGRATED_DENSITY = 25, CHANNEL = 26,
			SLICE = 27, FRAME = 28, FERET_X = 29, FERET_Y = 30, FERET_ANGLE = 31, MIN_FERET = 32, ASPECT_RATIO = 33,
			ROUNDNESS = 34, SOLIDITY = 35, MIN_THRESHOLD = 36, MAX_THRESHOLD = 37, LAST_HEADING = 37;
	private static final String[] defaultHeadings = { "Area", "Mean", "StdDev", "Mode", "Min", "Max", "X", "Y", "XM",
			"YM", "Perim.", "BX", "BY", "Width", "Height", "Major", "Minor", "Angle", "Circ.", "Feret", "IntDen",
			"Median", "Skew", "Kurt", "%Area", "RawIntDen", "Ch", "Slice", "Frame", "FeretX", "FeretY", "FeretAngle",
	...
	/**
	 * Constructs an empty ResultsTable with the counter=0, no columns and the
	 * precision set to 3 or the "Decimal places" value in Analyze/Set Measurements
	 * if that value is higher than 3.
	 */
	public ResultsTable() {
		init();
	}
	...
	/**
	 * Adds a numeric value to the specified column, on the last table row. Use
	 * addRow() to add another row to the table.
	 * 
	 * @see #addRow
	 * @see #addValue(String,double)
	...
	/**
	 * Adds a numeric value to the specified column, on the last table row. If the
	 * column does not exist, it is created. Use addRow() to add another row to the
	 * table.
	 * <p>
	 * JavaScript example:
	 * 
	 * <pre>
	 * rt = new ResultsTable(); for (n=0; n<=2*Math.PI; n+=0.1) { rt.addRow();
	 * rt.addValue("n", n); rt.addValue("Sine(n)", Math.sin(n));
	 * rt.addValue("Cos(n)", Math.cos(n)); } rt.show("Sine/Cosine Table");
	 * 
	 * </pre>
	 * 
	 * @see #addRow
	...
	/**
	 * Adds a string value to the specified column, on the last table row. If the
	 * column does not exist, it is created. Use addRow() to add another row to the
	 * table.
	 * 
	 * @see #addRow
	...
	/**
	 * Adds a label to the beginning of the specified row, or updates an existing
	 * lable, where 0<=row<size(). After labels are added or modified, call
	 * <code>show()</code> to update the window displaying the table.
	 */
	...
	/**
	 * Returns a copy of the given column as a double array, or null if the column
	 * is not found.
	 */
	...
	/**
	 * Returns a copy of the given column as a String array, or null if the column
	 * is not found.
	 */
	...
	/**
	 * Returns a copy of the given column as a float array, or null if the column is
	 * empty.
	 */
	...
	/**
	 * Sets the heading of the the first available column and returns that column's
	 * index. Returns COLUMN_IN_USE if this is a duplicate heading.
	 */
	...
	/**
	 * Returns the value of the given column and row, where column must be less than
	 * or equal the value returned by getLastColumn() and row must be greater than
	 * or equal zero and less than the value returned by size().
	 */
	...
	/**
	 * Returns the value of the specified column and row, where column is the column
	 * heading and row is a number greater than or equal zero and less than value
	 * returned by size(). Throws an IllegalArgumentException if this ResultsTable
	 ...
	 * greater than or equal zero and less than the value returned by size().
	 */
	...
	/**
	 * Returns the string value of the given column and row, where column must be
	 ...
	 * If the specified column does not exist, it is created. When adding columns,
	 * <code>show()</code> must be called to update the window that displays the
	 * table.
	 */
	 ...
	 * 0&lt;=column&lt;=(lastRow+1 and 0&lt;=row&lt;=size().
	 */
	...
	/**
	 * Sets the string value of the given column and row, where where
	 * 0&lt;=row&lt;size(). If the specified column does not exist, it is created.
	 * When adding columns, <code>show()</code> must be called to update the window
	...
	/**
	 * Returns a tab or comma delimited string representing the given row, where
	 * 0<=row<=size()-1.
	 */
	...
	/**
	 * @deprecated Replaced by addValue(String,double) and
	...
	/**
	 * Sets the decimal places (digits to the right of decimal point) that are used
	 * when this table is displayed.
	 */
	...
	/**
	 * This is a version of IJ.d2s() that uses scientific notation for small numbes
	 * that would otherwise display as zero.
	 */
	 ...
	 * title, or updates an existing results window. Opens a new window if there is
	 * no open text window with this title. The title must be "Results" if this
	 * table was obtained using ResultsTable.getResultsTable() or
	 ...
	 * table is empty. Displays an error message and returns 'false' if there is an
	 * error.
	 */
	 ...
	 * names as given by getHeadingsAsVaribleNames(). New variables starting with an
	 * uppercase letter create a new column with this name. The variable 'row' (the
	 * row index) is pre-defined. Except for the row label (if existing), currently
	 * only supports numeric values, no Strings.
	 * 
	 ...
	 * converted to valid variable names by replacing non-fitting characters with
	 * underscores and adding underscores. To make unique names, underscores+numbers
	...
	/**
	 * Sorts this table on the specified column, with string support. Author:
	 * 'mountain_man', 8 April 2019
	 */
