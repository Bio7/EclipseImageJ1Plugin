...
public class Opener {
	...
	public static final int UNKNOWN = 0, TIFF = 1, DICOM = 2, FITS = 3, PGM = 4, JPEG = 5, GIF = 6, LUT = 7, BMP = 8,
			ZIP = 9, JAVA_OR_TEXT = 10, ROI = 11, TEXT = 12, PNG = 13, TIFF_AND_DICOM = 14, CUSTOM = 15, AVI = 16,
			OJJ = 17, TABLE = 18, RAW = 19; // don't forget to also update 'types'
	public static final String[] types = { "unknown", "tif", "dcm", "fits", "pgm", "jpg", "gif", "lut", "bmp", "zip",
			"java/txt", "roi", "txt", "png", "t&d", "custom", "ojj", "table", "raw" };
	...
	private boolean isText(String path) {
		if (path.endsWith(".txt") || path.endsWith(".ijm") || path.endsWith(".java") || path.endsWith(".js")
				|| path.endsWith(".html") || path.endsWith(".htm") || path.endsWith(".bsh") || path.endsWith(".py")
				|| path.endsWith("/"))
			return true;
	...
	boolean allSameSizeAndType(FileInfo[] info) {
		...
		for (int i = 1; i < info.length; i++) {
			sameSizeAndType &= info[i].fileType == info[0].fileType && info[i].width == info[0].width
