...
public class Opener {
	...
	public static final int UNKNOWN = 0, TIFF = 1, DICOM = 2, FITS = 3, PGM = 4, JPEG = 5, GIF = 6, LUT = 7, BMP = 8, ZIP = 9, JAVA_OR_TEXT = 10, ROI = 11, TEXT = 12, PNG = 13, TIFF_AND_DICOM = 14,
			CUSTOM = 15, AVI = 16, OJJ = 17, TABLE = 18, RAW = 19; // don't forget to also update 'types'
		...
		try {
			EventQueue.invokeAndWait(new Runnable() {
				public void run() {
					...
					if (IJ.debugMode)
						IJ.log("Opener.openMultiple: " + sdir + " " + dir);
	...
	private boolean isText(String path) {
		if (path.endsWith(".txt") || path.endsWith(".ijm") || path.endsWith(".java") || path.endsWith(".js") || path.endsWith(".html") || path.endsWith(".htm") || path.endsWith(".bsh")
				|| path.endsWith(".py") || path.endsWith("/"))
			return true;
	...
	public ImagePlus openImage(String directory, String name) {
		...
		if (IJ.debugMode)
			IJ.log("openImage: \"" + types[this.fileType] + "\", " + path);
		switch (this.fileType) {
		case TIFF:
	...
	public ImagePlus openWithHandleExtraFileTypes(String path, int[] fileTypes) {
		...
		try {
			...
			if (IJ.debugMode)
				IJ.log("openWithHandleExtraFileTypes:\n" + path + "\n" + e);
			imp = null;
		}
		if (imp == null)
			return null;
	...
	ImagePlus openZipUsingUrl(URL url) throws IOException {
		...
		// throw new IOException("This ZIP archive does not appear to contain a .tif or
		// .dcm file\n"+name);
	...
	ImagePlus openJpegOrGifUsingURL(String title, URL url) {
		if (IJ.debugMode)
			IJ.log("openJpegOrGifUsingURL: " + url);
		if (url == null)
			return null;
	...
	public static ImagePlus openUsingImageIO(String path) {
		...
		if (img == null)
			return null;
		if (IJ.debugMode)
			IJ.log("type=" + img.getType() + ", alpha=" + img.getColorModel().hasAlpha() + ", bands=" + img.getSampleModel().getNumBands());
	...
	public ImagePlus openTiffStack(FileInfo[] info) {
		...
		if (fi.nImages > 1)
			...
			if (stack.size() == 0)
				return null;
			if (fi.fileType == FileInfo.GRAY16_UNSIGNED || fi.fileType == FileInfo.GRAY12_UNSIGNED || fi.fileType == FileInfo.GRAY32_FLOAT || fi.fileType == FileInfo.RGB48
	...
	/*
	 * public static boolean isRGBStack(ImagePlus imp) { if (imp==null) return
	 * false; boolean rgb = imp.getStackSize()==3 && imp.getNChannels()==3 &&
	 * imp.getBitDepth()==8; if (!rgb) return false; for (int i=1; i<=3; i++) {
	 * imp.setSlice(i); LUT lut = imp.getProcessor().getLut(); if (lut==null) { rgb
	 * = false; break; } byte[] bytes = lut.getBytes(); if (bytes==null) { rgb =
	 * false; break; } if (!((i==0 && (bytes[255]&255)==255 && (bytes[511]&255)==0
	 * && (bytes[767]&255)==0) || (i==1 && (bytes[255]&255)==0 &&
	 * (bytes[511]&255)==255 && (bytes[767]&255)==0) || (i==2 && (bytes[255]&255)==0
	 * && (bytes[511]&255)==0 && (bytes[767]&255)==255))) { rgb = false; break; } }
	 * imp.setSlice(1); return rgb; }
	 */
