...
public class RoiManager extends PlugInFrame implements ActionListener, ItemListener, MouseListener, MouseWheelListener, ListSelectionListener, Iterable<Roi> {
	...
	void showWindow() {
		...
		if (Util.isThemeBlack()) {
			list.setBackground(Util.getSWTBackgroundToAWT());
			list.setForeground(Util.getSWTForegroundToAWT());
			if (IJ.isLinux()) {
				list.setSelectionBackground(Util.getSWTForegroundToAWT());
				list.setSelectionForeground(Util.getSWTBackgroundToAWT());
			}
		}
	...
	void addPopupMenu() {
		pm = new PopupMenu();
		/* Changed for Bio7! */
		pm.setFont(Util.getOSFontToAwt());
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (label.equals(moreButtonLabel)) {
			/* Changed for Bio7 */
			Point ploc = panel.getLocationOnScreen();
			Point bloc = moreButton.getLocationOnScreen();
			ImageJ instance = IJ.getInstance();
			ImageJContextMenuAction icma = instance.getImageContextMenuAction();
			icma.swtPopupMenu(e, pm, new Point(ploc.x, bloc.y), this);
		}
		actionPerformed(label);
	}

	/* Changed for Bio7 - see above! */
	public void actionPerformed(String label) {
		// String label = e.getActionCommand();
		if (label == null)
			return;
	...
	/**
	 * Adds the specified ROI to the list. The second argument ('n') will be used to
	 * form the first part of the ROI label if it is zero or greater.
	 * 
	...
	/**
	 * Opens a single .roi file or a ZIP-compressed set of ROIs. Returns 'true' if
	...
	/**
	 * If one ROI is selected, it is saved as a .roi file, if multiple (or no) ROIs
	 * are selected, they are saved as a .zip ROI set. Returns 'true' if the save
	...
	/**
	 * This method measures the selected ROIs, or all ROIs if none are selected, on
	 * all the slices of a stack and returns a ResultsTable arranged with one row
	 * per slice.
	 * 
	 * @see <a href=
	 ...
	 * the results with one line per slice. By contrast, the measure() method
	...
	public void multiCrop(String directory, String options) {
		...
		if (options.contains("save")) {
			...
			/*
			 * if (options.contains("show")) { int width = 1; int height = 1; for (int i=0;
			 * i<rois.length; i++) { Rectangle bounds = rois[i].getBounds(); if
			 * (bounds.width>width) width = bounds.width; if (bounds.height>height) height =
			 * bounds.height; } imp = FolderOpener.open(directory, width, height,
			 * "virtual"); if (imp!=null) imp.show(); }
			 */
		}
	}
	...
	/**
	 * Returns a reference to the ROI Manager if it open or opens it if it is not
	 * already open.
	 */
	...
	/**
	 * Returns a reference to the ROI Manager, or null if it is not open and a batch
	 * mode macro is not running. If the ROI Manager is not open and a batch mode
	 * macro is running, returns the hidden batch mode RoiManager.
	 * 
	 * @see #getRoiManager
	 */
	 ...
	 * out of range. See also: RoiManager.getName() macro function.
	 */
	...
	/**
	 * Returns the index of the first ROI with the specified name, or -1 if no ROI
	 * has that name. See also: RoiManager.getIndex() macro function.
	 */
	 ...
	 * "Update", "Delete", "Measure", "Draw", "Show All", "Show None", "Fill",
	 * "Deselect", "Select All", "Combine", "AND", "XOR", "Split", "Sort" or "Multi
	 * Measure" command.
	 */
	...
	/**
	 * Adds the current selection to the ROI Manager, using the specified color (a 6
	...
	/**
	 * Selects multiple ROIs, where 'indexes' is an array of integers, each greater
	 * than or equal to 0 and less than the value returned by getCount().
	 * 
	...
	/**
	 * This is a macro-callable version of getSelectedIndexes(). Example:
